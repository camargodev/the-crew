...
class GameEngine:
    ...
    def __init__(self, player_interface: PlayerInterface, round_engine: RoundEngine, card_dealer: CardDealer):
        self.player_interface = player_interface
        self.round_engine = round_engine
        self.card_dealer = card_dealer

    def play_game(self, players: list[Player], missions: list[MissionRule]):
        ...
        for _ in range(number_of_rounds):
            ...
            missions_data, is_game_over = self.__validate_missions__(missions_data, game_data)
            if is_game_over:
                break
                
        return game_data, missions_data
    
    def __validate_missions__(self, missions_data: GameMissionsData, game_data: GameData):
